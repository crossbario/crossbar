...
from twisted.internet.defer import Deferred, gatherResults, inlineCallbacks, returnValue, succeed
...
class Node(object):
    ...
    def _add_global_roles(self):
        ...
        self.log.info(
            '{func} node-wide role "{authrole}" added on node management router realm "{realm}"',
    ...
    def set_service_session(self, session, realm, authrole=None):
        self.log.info(
            '{func}(session={session}, realm="{realm}", authrole="{authrole}")',
    ...
    def start(self, node_id=None):
        ...
        """
        self.log.info(
            "{note} [{method}]", note=hl("Starting node ..", color="green", bold=True), method=hltype(Node.start)
        )
        ...
        self.log.info(
            "Node ID {node_id} set from {node_id_source}", node_id=hlid(self._node_id), node_id_source=_node_id_source
        )
        ...
        self.log.info(
            "{func} router service agent session attached [{router_service}]",
        ...
        self.log.info(
            "{func} node controller session attached [{node_controller}]",
            func=hltype(self.start),
    ...
    def boot_from_config(self, config):
        ...
        if len(workers):
            self.log.info(
                hl(
                    "Will start {} worker{} ..".format(len(workers), "s" if len(workers) > 1 else ""),
                    color="green",
                    bold=True,
                )
            )
        ...
        for worker in workers:
            ...
            if worker_type in self._native_workers:
                ...
                if worker_disabled:
                    ...
                    d = self._controller.call(
                        "crossbar.start_worker", worker_id, worker_type, worker_options, options=CallOptions()
                    )
                    ...
                    def configure_worker(res, worker_logname, worker_type, worker_id, worker):
                        ...
                        try:
                            ...
                            raise ValueError(
                                "A native worker of type '{}' is configured but there is no method '{}' on {}".format(
            ...
            # guest worker processes setup
                ...
                d = self._controller.call(
                    "crossbar.start_worker", worker_id, worker_type, worker, options=CallOptions()
                )
    ...
    def _configure_native_worker_common(self, worker_logname, worker_id, worker):
        ...
        if "manhole" in worker:
            yield self._controller.call(
                "crossbar.worker.{}.start_manhole".format(worker_id), worker["manhole"], options=CallOptions()
            )
    ...
    def _configure_native_worker_router(self, worker_logname, worker_id, worker):
        ...
        for realm in worker.get("realms", []):
            ...
            yield self._controller.call(
                "crossbar.worker.{}.start_router_realm".format(worker_id), realm_id, realm, options=CallOptions()
            )
    ...
    def _configure_native_worker_container(self, worker_logname, worker_id, worker):
        ...
        for component in worker.get("components", []):
            ...
            yield self._controller.call(
                "crossbar.worker.{}.start_component".format(worker_id), component_id, component, options=CallOptions()
            )
            self.log.info(
                "{worker}: component '{component_id}' started", worker=worker_logname, component_id=component_id
            )
