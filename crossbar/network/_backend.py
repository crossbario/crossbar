...
from cfxdb.xbr import Actor, ActorType, Api, Catalog, Market
from cfxdb.xbrnetwork import Account, UserKey, VerifiedAction
from hexbytes import HexBytes
from twisted.internet.threads import deferToThread
from txaio import make_logger, sleep, time_ns
...
from ._mailgw import (
    _CREATE_CATALOG_LOG_VERIFICATION_CODE_END,
    _CREATE_CATALOG_LOG_VERIFICATION_CODE_START,
    _CREATE_MARKET_LOG_VERIFICATION_CODE_END,
    _CREATE_MARKET_LOG_VERIFICATION_CODE_START,
    _JOIN_MARKET_LOG_VERIFICATION_CODE_END,
    _JOIN_MARKET_LOG_VERIFICATION_CODE_START,
    _LOGIN_MEMBER_LOG_VERIFICATION_CODE_END,
    _LOGIN_MEMBER_LOG_VERIFICATION_CODE_START,
    _ONBOARD_MEMBER_LOG_VERIFICATION_CODE_END,
    _ONBOARD_MEMBER_LOG_VERIFICATION_CODE_START,
    _PUBLISH_API_LOG_VERIFICATION_CODE_END,
    _PUBLISH_API_LOG_VERIFICATION_CODE_START,
    MailGateway,
)
from ._util import alternative_username, hl, hlcontract, hlid, hlval
...
def _verify_meta_data(meta_hash, meta_data, meta_obj_expected):
    ...
    if meta_data is not None:
        ...
        if meta_hash != _meta_hash:
            raise RuntimeError(
                'Invalid meta_hash "{}": hash does not match expected "{}"'.format(meta_hash, _meta_hash)
            )
        ...
        if _meta_obj_received != meta_obj_expected:
            raise RuntimeError(
                "Invalid meta_data {} does not match expected data {}".format(_meta_obj_received, meta_obj_expected)
            )
...
class Backend(object):
    ...
    def __init__(
        ...
        db: zlmdb.Database,
        meta_schema: cfxdb.meta.Schema,
        xbr_schema: cfxdb.xbr.Schema,
        xbrnetwork_schema: cfxdb.xbrnetwork.Schema,
        chain_id: int,
        eth_privkey_raw: bytes,
        w3: web3.Web3,
        mailgw: MailGateway,
        bc_config: Dict,
        ipfs_cache_dir: str,
    ...
    def _save_verification_file(self, vaction_oid, vaction_type, verified_data):
        ...
        self.log.info(
            'New verification file "{verification_file}" written', verification_file=hlval(verification_file)
        )
    ...
    def _remove_verification_file(self, vaction_oid, vaction_type, rename_only=True):
        ...
        if os.path.isfile(verification_file):
            if rename_only:
                ...
                self.log.info(
                    'Verification file removed "{verification_file}"', verification_file=hlval(verification_file)
                )
                return True
        else:
            return False
    ...
    async def _ensure_runtime(self, ts_started):
        ...
        ts_ended = time_ns()
        duration_ms = int((ts_ended - ts_started) / 1000000.0)
        if duration_ms < 100:
            delay_secs = (100.0 - duration_ms) / 1000.0
            await sleep(delay_secs)
        else:
            self.log.warn(
                "excessive run-time of {duration_ms} ms in {klass}.{caller}",
    ...
    def _send_for(self, function, *args):
        ...
        self.log.info(
            "{klass}._send_for ({func}) [1/4] - Ethereum transaction nonce: nonce={nonce}",
    ...
    def _send_createMarketFor(
        ...
        :return:
        """
        return self._send_for(
            xbr.xbrmarket.functions.createMarketFor,
            member,
            created,
            marketId,
            coin,
    ...
    def _send_joinMarketFor(self, member, joined, marketId, actorType, meta, signature):
        ...
        """
        return self._send_for(
            xbr.xbrmarket.functions.joinMarketFor, member, joined, marketId, actorType, meta, signature
        )

    def _send_createCatalogFor(self, member, created, catalog_id, terms, meta, signature):
        return self._send_for(
            xbr.xbrcatalog.functions.createCatalogFor, member, created, catalog_id, terms, meta, signature
        )
    ...
    async def onboard_member(
        ...
        if profile_data is not None:
            ...
            if profile_hash != _profile_hash:
                raise RuntimeError(
                    'Invalid profile_hash "{}": hash does not match expected "{}"'.format(profile_hash, _profile_hash)
                )
            ...
            if _profile_obj_received != _profile_obj_expected:
                raise RuntimeError(
                    "Invalid profile_data {} does not match expected data {}".format(
        ...
        try:
            signer_address = recover_eip712_member_register(
                chain_id, contract_adr, wallet_adr, block_number, eula_hash, profile_hash, signature
            )
        except Exception as e:
            self.log.warn(
                "EIP712 signature recovery failed (wallet_adr={wallet_adr}): {err}", wallet_adr=wallet_adr, err=str(e)
            )
            raise ApplicationError(
                "xbr.network.error.invalid_signature", "EIP712 signature recovery failed ({})".format(e)
            )
        ...
        with self._db.begin() as txn:
            ...
            if account_oid:
                ...
                timestamp = time_ns()
                return await self._really_login(
                    account_oid, member.email, client_pubkey, timestamp, wallet_adr, ts_started, was_signup=True
                )
            ...
            if account_oid:
                ...
                if member.wallet_address == wallet_adr:
                    ...
                    self._mailgw.send_wrong_wallet_email(
                        member_email, with_0x(address_hex), with_0x(binascii.b2a_hex(wallet_adr).decode())
                    )
        ...
        if not member_email.endswith("@nodomain"):  # filter CI/test scripts data and skip sending emails!
            ...
            self.log.warn(
                'Sending of email to "{member_email}" skipped - domain is filtered', member_email=member_email
            )
    ...
    async def login_member(
        self, member_email, client_pubkey, chain_id, block_number, contract_adr, timestamp, wallet_adr, signature
    ):
        """
        ...
        try:
            signer_address = recover_eip712_member_login(
                chain_id, contract_adr, wallet_adr, block_number, timestamp, member_email, client_pubkey, signature
            )
        except Exception as e:
            self.log.warn(
                "EIP712 signature recovery failed (wallet_adr={wallet_adr}): {err}", wallet_adr=wallet_adr, err=str(e)
        ...
        with self._db.begin() as txn:
            ...
            if account_oid:
                ...
                if member.email != member_email:
                    raise ApplicationError(
                        "xbr.network.error.invalid_email", "Provided email not associated to the member"
                    )
            ...
            if member_address != wallet_adr:
                ...
                raise ApplicationError(
                    "xbr.network.error.invalid_wallet", "Signer address does not match member's address"
                )

        return await self._really_login(account_oid, member_email, client_pubkey, timestamp, wallet_adr, ts_started)

    async def _really_login(
        self, account_oid, member_email, client_pubkey, timestamp, wallet_adr, ts_started, was_signup=False
    ):
        # create new verification action ID and code
        vaction_oid = uuid.uuid4()
        ...
        # send email with verification link, containing the verification code (email is the 2nd channel here)
        if not member_email.endswith("@nodomain"):  # filter CI/test scripts data and skip sending emails!
            self._mailgw.send_login_verification(
                member_email,
                vaction_oid,
                vaction_code,
    ...
    async def get_member(self, member_oid):
        ...
        def do_get_member(wallet_adr):
            ...
            member_registered, member_profile, member_eula, member_level, member_signature = (
                xbr.xbrnetwork.functions.members(wallet_adr).call()
            )
    ...
    def get_block(self, block_no):
        ...
        :return:
        """
        assert isinstance(block_no, bytes) and len(block_no) == 32, 'block_no must be bytes[32], was "{}"'.format(
            block_no
        )
    ...
    async def get_market(self, market_oid, include_attributes, include_terms_text):
        ...
        with self._db.begin() as txn:
            ...
            if include_attributes:
                ...
                for attribute in self._meta.attributes.select(
                    txn, return_keys=False, from_key=from_key, to_key=to_key
                ):
    ...
    async def create_market(
        ...
        try:
            signer_address = recover_eip712_market_create(
                chain_id,
                contract_adr,
                member_adr,
                block_number,
            ...
            self.log.warn(
                "EIP712 signature recovery failed (member_adr={member_adr}): {err}", member_adr=member_adr, err=str(e)
            )
            raise ApplicationError(
                "xbr.network.error.invalid_signature", "EIP712 signature recovery failed ({})".format(e)
            )
        ...
        with self._db.begin() as txn:
            ...
            if market_oid:
                raise ApplicationError(
                    "xbr.network.error.maker_already_working_for_other_market",
        ...
        for k in verified_data:
            ...
            if type(v) is type(memoryview):
                ...
                self.log.warn(
                    'Monkey-patched (memoryview => bytes) dict "verified_data" for key "{key_name}"', key_name=k
                )
    ...
    async def join_market(
        self, member_id, market_id, chain_id, block_number, contract_adr, actor_type, meta_hash, meta_data, signature
    ):
        """
        ...
        try:
            signer_address = recover_eip712_market_join(
                chain_id, contract_adr, member_adr, block_number, market_id.bytes, actor_type, meta_hash, signature
            )
        except Exception as e:
            self.log.failure()
            self.log.warn(
    ...
    async def verify_join_market(self, vaction_oid, vaction_code):
        ...
        try:
            ...
            signature = vaction.verified_data["signature"]
            txn_hash = await deferToThread(
                self._send_joinMarketFor, member, joined, marketId, actorType, meta, signature
            )
        except Exception as e:
    ...
    async def create_catalog(
        ...
        try:
            signer_address = recover_eip712_catalog_create(
                verifying_chain_id,
    ...
    async def verify_create_catalog(self, vaction_oid, vaction_code):
        ...
        try:
            ...
            txn_hash = await deferToThread(
                self._send_createCatalogFor, member, created, catalog_oid, terms_hash, meta, signature
            )
        except Exception as e:
    ...
    def publish_api(
        ...
        try:
            signer_address = recover_eip712_api_publish(
                verifying_chain_id,
                verifying_contract_adr,
                member_adr,
                current_block_number,
