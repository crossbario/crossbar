...
class ProxyFrontendSession(object):
    ...
    def _process_Hello(self, msg):
        ...
        self._send_if_transport_is_alive(message.Abort(ApplicationError.NO_AUTH_METHOD, message='no suitable authmethod found'))
                ...
                if not self.transport:
                    ...
                    if isinstance(auth_result, types.Accept):
                        try:
                            ...
                            self.log.info('{func} frontend disconnected while connecting backend session {pending_auth}',
                                          func=hltype(self._process_Authenticate),
                                          pending_auth=self._pending_auth)
                        except Exception as e:
                            self.log.failure()
                        ...
                        self._send_if_transport_is_alive(message.Abort(auth_result.reason, message=auth_result.message))
                    else:
        ...
        try:
            ...
            if self._service_sessions[realm] is not None and authrole not in self._service_sessions[realm]:
                if self.has_role(realm, authrole):
                    ...
                    self._service_sessions[realm][authrole] = make_service_session(self._reactor, self, backend_config, realm, authrole)
                else:
                    # mark as non-existing!
