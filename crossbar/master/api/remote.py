...
class RemoteApi(object):
    ...
    def subscribe(self, session):
                ...
                if details:
                    ...
                    if match:
                        ...
                        return session.publish(
                            local_uri, node_id, *args, **kwargs, options=PublishOptions(exclude_me=False)
                        )
            ...
            def forward(*args, **kwargs):
                ...
                if details:
                    ...
                    if match:
                        ...
                        return session.publish(
                            local_uri, node_id, worker_id, *args, **kwargs, options=PublishOptions(exclude_me=False)
                        )
        ...
        for topic in topics_by_worker:
            ...
            remote_uri_regex = (
                r"^crossbarfabriccenter.node.([a-z0-9][a-z0-9_\-]*).worker.([a-z0-9][a-z0-9_\-]*).{topic}$".format(
                    topic=rtopic
                )
            )
