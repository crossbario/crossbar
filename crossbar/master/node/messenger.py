...
class Messenger(object):
    ...
    def send_message(self, receiver, subject, text):
        self.log.info(
            'sending mail via mailgun: receiver={receiver}, subject="{subject}", textlen={textlen}',
            receiver=receiver,
            subject=subject,
            textlen=len(text),
        )
        ...
        try:
            if self._access_key and self._submit_url:
                ...
                self.log.warn(
                    "Mailgun not configured! This is the mail that would have been sent: {mail}", mail=json.dumps(data)
                )
                res = None
