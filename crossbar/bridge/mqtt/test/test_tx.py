...
class TwistedProtocolTests(TestCase):
    ...
    def test_lose_conn_on_reserved_qos3(self):
        ...
        pub = Publish(
            duplicate=False, qos_level=3, retain=False, topic_name="foo", packet_identifier=1, payload=b"bar"
        )
...
class PublishHandlingTests(TestCase):
    def test_qos_0_sends_no_ack(self):
        ...
        pub = Publish(
            duplicate=False, qos_level=0, retain=False, topic_name="foo", packet_identifier=None, payload=b"bar"
        ).serialise()
    ...
    def test_qos_1_sends_ack(self):
        ...
        pub = Publish(
            duplicate=False, qos_level=1, retain=False, topic_name="foo", packet_identifier=1, payload=b"bar"
        ).serialise()
    ...
    def test_qos_2_sends_ack(self):
        ...
        pub = Publish(
            duplicate=False, qos_level=2, retain=False, topic_name="foo", packet_identifier=1, payload=b"bar"
        ).serialise()
...
class SendPublishTests(TestCase):
    ...
    def test_qos_1_queues_message(self):
        ...
        expected_publish = Publish(
            duplicate=False, qos_level=1, retain=False, packet_identifier=1, topic_name="hello", payload=b"some bytes"
        )
        events = cp.data_received(t.value())
        t.clear()
        self.assertEqual(len(events), 1)
    ...
    def test_qos_2_queues_message(self):
        ...
        # We should now get the sent Publish
        expected_publish = Publish(
            duplicate=False, qos_level=2, retain=False, packet_identifier=1, topic_name="hello", payload=b"some bytes"
        )
        events = cp.data_received(t.value())
        t.clear()
        self.assertEqual(len(events), 1)
    ...
    def test_qos_1_resent_on_disconnect(self):
        ...
        resent_publish = Publish(
            duplicate=True, qos_level=1, retain=False, packet_identifier=1, topic_name="hello", payload=b"some bytes"
        )
    ...
    def test_qos_2_resent_on_disconnect_pubrel(self):
        ...
        # The Publish packet must have DUP set to True.
        resent_publish = Publish(
            duplicate=True, qos_level=2, retain=False, packet_identifier=1, topic_name="hello", payload=b"some bytes"
        )
        self.assertEqual(events[1], resent_publish)
